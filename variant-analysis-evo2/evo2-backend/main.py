...
evo2_image = (
    ...
    # 1. Install Torch AND ALL Build Tools (packaging, ninja, wheel)
    #    Added 'wheel' here to fix the latest error.
    .pip_install("torch==2.2.0", "packaging", "ninja", "wheel", "setuptools")
    
    .run_commands(
        "git clone https://github.com/Zymrael/vortex.git",
        "cd vortex && pip install --no-deps -e ."
    )

    # 2. Install Evo2
    .run_commands(
        "git clone --recurse-submodules https://github.com/ArcInstitute/evo2.git",
        "cd evo2 && pip install --no-deps -e ."
    )
    
    # 3. Install Flash Attention with --no-build-isolation
    #    Now it will find 'wheel' and compile correctly.
    .env({"MAX_JOBS": "1"})
