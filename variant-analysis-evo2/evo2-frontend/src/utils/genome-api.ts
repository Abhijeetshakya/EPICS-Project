      ...
      if (i < data[3].length) {
        try {
          ...
          results.push({
            ...
            name: display[2],   // Description is at index 2 (df order)
            chrom,
...
export async function analyzeVariantWithEvo2(
  position: number,
  alternative: string,
  genome: string,
  chromosome: string,
  ...
  try {
    // Call our NextJS API route (which acts as a proxy to bypass CORS)
    ...
    return {
      ...
      delta_score: data.delta_score || data.score || 0,
      prediction: data.classification || "UNKNOWN",
      classification_confidence: 1 - Math.exp(-Math.abs(data.delta_score || data.score || 0)),
    };
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    console.error("EVO2 analysis error:", errorMsg);
    throw new Error(`Failed to analyze variant: ${errorMsg}`);
  }
}

export async function checkEvo2Health(): Promise<boolean> {
  try {
    // Check via our API route
...
export async function fetchClinicalContext({
  ...
  variantId?: string;
  prediction?: string;
}): Promise<ClinicalContextResult> {
  try {
    const response = await fetch("http://localhost:8000/analyze-clinical-context", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    console.error("Clinical context fetch error:", error);
    return {
      found: false,
      summary: "Failed to retrieve clinical context.",
      sources: []
    };
  }
}
