...
import { ArrowLeft, MapPin, Ruler, GitBranch, Layers, FlaskConical } from "lucide-react";
...
export default function GeneViewer({
  ...
  const fetchGeneSequence = useCallback(
    async (start: number, end: number) => {
      try {
        ...
        setError(null);
        const { sequence, actualRange: fetchedRange, error: apiError } =
  ...
  useEffect(() => {
    const initializeGeneData = async () => {
      ...
      try {
        const { geneDetails: fetchedDetail, geneBounds: fetchedGeneBounds, initialRange: fetchedRange } =
  ...
  const handleLoadSequence = useCallback(() => {
    ...
    if (isNaN(start) || isNaN(end)) {
      ...
      if (start < minBound) validationError = `Start position below minimum (${minBound.toLocaleString()})`;
      else if (end > maxBound) validationError = `End position exceeds maximum (${maxBound.toLocaleString()})`;
      if (end - start > 10000) validationError = `Range exceeds maximum view of 10,000 bp.`;
    }
  ...
  useEffect(() => {
    if (geneBounds) fetchClinvarVariants();
  }, [geneBounds]);
  ...
  // Derived stats
  const geneSize = geneBounds ? Math.abs(geneBounds.max - geneBounds.min + 1) : 0;
  const pathogenicCount = clinvarVariants.filter(v => v.classification?.toLowerCase().includes("pathogenic")).length;
  const benignCount = clinvarVariants.filter(v => v.classification?.toLowerCase().includes("benign")).length;
  const vusCount = clinvarVariants.filter(v =>
    v.classification?.toLowerCase().includes("uncertain") || v.classification?.toLowerCase().includes("conflicting")
  ).length;
  const strandDir = geneDetail?.genomicinfo?.[0]?.strand === "-" ? "Reverse (−)" : "Forward (+)";
    ...
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-500">Loading gene data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <Button
        variant="ghost"
        size="sm"
        className="cursor-pointer text-slate-400 hover:text-slate-200 hover:bg-slate-800/50"
        onClick={onClose}
      >
      ...
      {/* ─── Gene Stats Dashboard ────────────────────────────────────── */}
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <div className="stat-pill">
          <div className="flex items-center gap-2 mb-1">
            <FlaskConical className="h-3.5 w-3.5 text-teal-400" />
            <span className="text-[10px] uppercase tracking-wider text-slate-500">Gene</span>
          </div>
          <p className="text-lg font-bold text-teal-300">{gene.symbol}</p>
        </div>

        <div className="stat-pill">
          <div className="flex items-center gap-2 mb-1">
            <MapPin className="h-3.5 w-3.5 text-sky-400" />
            <span className="text-[10px] uppercase tracking-wider text-slate-500">Chromosome</span>
          </div>
          <p className="text-lg font-bold text-sky-300">{gene.chrom}</p>
        </div>
        ...
        <div className="stat-pill">
          ...
          <p className="text-lg font-bold text-violet-300">{geneSize > 1000 ? `${(geneSize / 1000).toFixed(1)}kb` : `${geneSize}bp`}</p>
        </div>

        <div className="stat-pill">
          <div className="flex items-center gap-2 mb-1">
            <GitBranch className="h-3.5 w-3.5 text-amber-400" />
            <span className="text-[10px] uppercase tracking-wider text-slate-500">Strand</span>
          </div>
          <p className="text-lg font-bold text-amber-300">{strandDir}</p>
        </div>

        <div className="stat-pill">
          <div className="flex items-center gap-2 mb-1">
            <Layers className="h-3.5 w-3.5 text-rose-400" />
            <span className="text-[10px] uppercase tracking-wider text-slate-500">Variants</span>
          </div>
          <p className="text-lg font-bold text-rose-300">{clinvarVariants.length}</p>
          {clinvarVariants.length > 0 && (
            <div className="flex gap-2 mt-1">
              <span className="text-[10px] text-red-400">{pathogenicCount}P</span>
              <span className="text-[10px] text-emerald-400">{benignCount}B</span>
              <span className="text-[10px] text-yellow-400">{vusCount}V</span>
            </div>
          )}
        </div>
        ...
        <div className="stat-pill">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] uppercase tracking-wider text-slate-500">Assembly</span>
          </div>
          <p className="text-lg font-bold text-slate-300">{genomeId}</p>
        </div>
      </div>
